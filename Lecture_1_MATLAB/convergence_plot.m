...
%% Plot convergence with reference line for 2nd order
figure;
loglog(N_list, rel_L2_errors, 'o-', 'LineWidth', 3, 'MarkerSize', 8);
hold on;

% Reference line: choose first point and slope -2
ref_slope = rel_L2_errors(1) * (N_list/N_list(1)).^(-2);
loglog(N_list, ref_slope, '--r', 'LineWidth', 3);

grid on;
xlabel('N (interior points per dimension)');
ylabel('Relative L2 Error');
title('Convergence of 5-point stencil for 2D Poisson');
legend('Numerical Error', 'O(h^{-2}) Reference', 'Location', 'southwest');

%% ================= Local Function for Solving Poisson Equation ===================
function rel_L2 = convergence_l2(N)
    ...
    f = -2*pi^2 * sin(pi*X).*sin(pi*Y);   % -Δu = f
